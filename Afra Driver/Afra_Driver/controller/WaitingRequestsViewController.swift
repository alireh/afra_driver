//
//  WaitingRequestsViewController.swift
//  Afra_Driver
//
...
class WaitingRequestsViewController: UIViewController, UITableViewDataSource, UITableViewDelegate {
    
    @IBOutlet weak var backPanel: BackPanel!
    @IBOutlet weak var requestTable: UITableView!
    @IBOutlet weak var loadingView: UIActivityIndicatorView!
    
    var selectdeRequestInfo = activeRequestInfo()
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        if let destinationVC = segue.destination as? DisplayUserTrashesViewController {
            destinationVC.requestInfo = selectdeRequestInfo
        }
    }
    ...
    {
        if let cell = tableView.dequeueReusableCell(withIdentifier: "WaitingRequestCell") as? WaitingRequestCell {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        return WaitingRequestCell()
    }
    
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        let req = requests[indexPath.row]
        
        let cellHeight = 150.0 + CGFloat(45.0 * Double((req.trashes?.count)!))
        req.cellHeight = cellHeight
        selectdeRequestInfo = req
        performSegue(withIdentifier: TO_DISPLAY_USER_TRASH_SEGUE, sender: nil)
    }
